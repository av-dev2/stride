...
def load_json(file):
	...
	with open(json_file_path) as f:  # nosemgrep: frappe-security-file-traversal
		data = json.load(f)
	return data
		...
		for doctype, fields in doctype_custom_fields_dict.items():
			for df in fields:
				...
				try:
					...
					print(
						f"WARNING [stride]: Failed to create custom field '{fieldname}' on '{doctype}': {e}",
						file=sys.stderr,
					)
					frappe.log_error(
						title=f"stride: custom field failed - {doctype}.{fieldname}",
						message=frappe.get_traceback(),
					)


def execute():
	# Read names of only JSON files in the custom_fields_json folder
	json_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), folder)
	...
	for file in files:
		try:
			...
			print(
				f"WARNING [stride]: Failed to process custom fields from '{file}': {e}",
				file=sys.stderr,
			)
			frappe.log_error(
				title=f"stride: custom field file failed - {file}",
				message=frappe.get_traceback(),
			)
