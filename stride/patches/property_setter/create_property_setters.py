...
def create_property_setter_from_json(property_setters_obj):
    ...
    for property_setter in property_setters_obj:
        ...
        try:
            ...
            print(
                f"WARNING [stride]: Failed to create property setter " f"'{ps_name}': {e}",
                file=sys.stderr,
            )
            frappe.log_error(
                title=f"stride: property setter failed - {ps_name}",
                message=frappe.get_traceback(),
            )


def execute():
    # Read names of only JSON files in the property_setters_json folder
    ...
    for file in files:
        try:
            ...
            print(
                f"WARNING [stride]: Failed to process property setters from '{file}': {e}",
                file=sys.stderr,
            )
            frappe.log_error(
                title=f"stride: property setter file failed - {file}",
                message=frappe.get_traceback(),
            )
